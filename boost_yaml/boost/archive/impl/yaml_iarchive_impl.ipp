...
yaml_iarchive_impl<Archive>::load(std::wstring &ws){
    std::string s;
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
yaml_iarchive_impl<Archive>::load(wchar_t * ws){
    std::string s;
    bool result = gimpl->parse_string(is, s);
    if(! result)
        boost::throw_exception(
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
yaml_iarchive_impl<Archive>::load(std::string &s){
    bool result = gimpl->parse_string(is, s);
    if(! result)
        boost::throw_exception(
            yaml_archive_exception(yaml_archive_exception::yaml_archive_parsing_error)
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
yaml_iarchive_impl<Archive>::load(char * s){
    std::string tstring;
...
template<class Archive>
BOOST_ARCHIVE_DECL(void)
yaml_iarchive_impl<Archive>::init(){
    gimpl->init(is);
...
yaml_iarchive_impl<Archive>::yaml_iarchive_impl(
    std::istream &is_,
    unsigned int flags
) :
