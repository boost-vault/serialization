...
// Modification in 2010 by Paul Fee
// Permit friend to serialize static_bloom_filter
...
namespace boost {
    ...
    namespace bloom_filters {
        template <typename Input, size_t M, typename HashFunctions>
        class static_bloom_filter;
    }
    ...
    namespace serialization {
      template<class Archive, typename Input, size_t M, typename HashFunctions>
        void serialize(
            Archive & ar,
            boost::bloom_filters::static_bloom_filter<Input, M, HashFunctions>& t,
    ...
    namespace bloom_filters {
        ...
        struct static_bloom_filter : protected detail::internals<Input, std::bitset<M> > {
            template <class Archive, typename S_Input, size_t S_M, typename S_HashFunctions>
                friend void boost::serialization::serialize(
                    Archive & ar,
                    static_bloom_filter<S_Input, S_M, S_HashFunctions>& t,
                    const unsigned int version);
